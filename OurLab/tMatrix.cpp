...
tMatrix & tMatrix :: operator=(const tMatrix M)
{
   if ( M.row_cnt() > this -> row_cnt() || M.col_cnt() > this -> col_cnt() )
	  throw "Can't assign matrix to a smaller matrix.";
...
tMatrix tMatrix :: Inv2(void)
{
   double det = this -> at(0, 0) * this -> at(1, 1) - this -> at(0, 1) * this -> at(1, 0);
   if ( is_zero(det) )
	  throw new tMatrExc("Matrix is degenerate!");
   if (this -> col_cnt() != 2 || this -> row_cnt() != 2)
      throw new tMatrExc("Must be a 2x2 matrix.");
   
   tMatrix Res(2, 2);
   Res(0, 0) = this -> at(1, 1) / det;
   Res(0, 1) = - this -> at(0, 1) / det;
   Res(1, 0) = - this -> at(1, 0) / det;
   Res(1, 1) = this -> at(0, 0) / det;
   return Res;
}
